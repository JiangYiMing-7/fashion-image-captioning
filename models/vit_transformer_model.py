...
from configs.model_configs import ViTTransformerModelConfig,TransformerDecoderConfig
...
class ViTTransformerModel(nn.Module):
    ...
    def __init__(self, config: ViTTransformerModelConfig, pad_token_id: int = 0) -> None:
        super().__init__()
        self.config = config
        self.encoder = ViTEncoder(config.encoder)
        self.decoder = TransformerDecoder(config.decoder, pad_token_id=pad_token_id)
...
def build_vit_transformer_model(
    ...
    decoder_cfg: TransformerDecoderConfig = model_cfg.decoder
    decoder_cfg.vocab_size = vocab_size
    return ViTTransformerModel(model_cfg, pad_token_id=pad_token_id)
